...
integrationTest.describe("Paddle cancel flow", () => {
  ...
  (["inline", "overlay"] as const).forEach((mode) => {
    integrationTest(
      `Returns to the paywall when closing the ${mode} checkout`,
      async ({ page, userId, email }) => {
        ...
        if (mode === "inline") {
          ...
          // The purchase promise rejects with a user-cancelled error and the
          // demo stays on the paywall with the checkout UI fully unmounted.
          await expect(
          ...
          // checkout.closed event, which the SDK maps to a user-cancelled
          // error.
          ...
          await expect(
            page.locator("iframe[src*='paddle.com']"),
          ).not.toBeVisible({ timeout: PADDLE_UI_STEP_TIMEOUT_MS });
        }
