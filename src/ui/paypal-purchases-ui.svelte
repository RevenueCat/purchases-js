<script lang="ts">
  ...
  let currentPage = $state<
    "waiting" | "loading" | "paypal-buttons" | "success" | "error"
  >("waiting");
  let paypalButtonContainer: HTMLElement | null = null;
  let resolveContainerReady: (() => void) | null = null;
  const containerReadyPromise = new Promise<void>((resolve) => {
    resolveContainerReady = resolve;
  });

  const onPayPalContainerReady = (el: HTMLElement) => {
    paypalButtonContainer = el;
  ...
  onMount(async () => {
    ...
    try {
      const result = await paypalService.purchase({
        ...
        onButtonsReady: () => {
          // Buttons are rendered, already showing paypal-buttons page
        },
        onClose,
        container: paypalButtonContainer!,
      });
