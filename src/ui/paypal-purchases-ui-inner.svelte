<script lang="ts">
  ...
  interface Props {
    currentPage: "loading" | "paypal-buttons" | "success" | "error";
    ...
    onContinue: () => void;
    closeWithError: () => void;
    onPayPalContainerReady: (el: HTMLElement) => void;
  }
  ...
  let paypalButtonContainer: HTMLElement | undefined = $state();

  const translator: Writable<Translator> = getContext(translatorContextKey);

  $effect(() => {
    if (paypalButtonContainer) {
      onPayPalContainerReady(paypalButtonContainer);
    }
  });
</script>
...
<FullscreenTemplate {brandingInfo} {isInElement} {isSandbox}>
  {#snippet mainContent()}
    ...
    {:else if currentPage === "paypal-buttons"}
      <div class="rcb-paypal-buttons-container">
        <div bind:this={paypalButtonContainer}></div>
      </div>
